...
.header-family {
  ...
  white-space: normal; font-size: 1rem; display:flex; align-items:center; justify-content:center; text-align:center;
}
.header-subfamily {
  font-weight: 700; background: #f0f2ff; padding: 8px; white-space: normal;
  font-size: 0.95rem; display:flex; align-items:center; justify-content:center; text-align:center;
}
.grade-header {
  font-weight: 800; font-size: 0.95rem; background: #1E56E0; color: #fff; padding: 8px;
  border-right: 2px solid #fff; display:flex; align-items:center; justify-content:center; position:sticky; top:0; left:0; z-index:40 !important;
}
.grade-cell {
...
.job-card {
  background:#fafafa; border-left:4px solid #1E56E0; border-radius:6px; padding:5px 8px; margin:3px 0; text-align:left;
  font-size:0.82rem; box-shadow:0 1px 2px rgba(0,0,0,0.05); white-space:normal;
}
...
if "job_profile" not in DATA:
    st.error("⚠️ Arquivo `data/Job Profile.xlsx` não encontrado.")
    st.stop()
...
df = df.dropna(subset=["Job Family", "Sub Job Family", "Job Profile"])
...
# GRADE HORIZONTAL
# ===========================================================
if "Global Grade" in filtered.columns:
    grades = sorted(
...
for g in grades:
    ...
    for f in families:
        ...
        sfs = subfam_map[f] or ["-"]
        for sf in sfs:
            cell_df = fam_df[(fam_df["Sub Job Family"] == sf)]
            if "Global Grade" in cell_df.columns:
                cell_df = cell_df[cell_df["Global Grade"] == g]
            if not cell_df.empty:
                cards = "".join([
                    f"<div class='job-card' title='{r.get('Full Job Code','-')}'><b>{r.get('Job Profile','-')}</b><span>{r.get('Career Path','-')}</span></div>"
