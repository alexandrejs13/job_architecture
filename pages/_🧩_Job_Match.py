...
# Versão aprimorada — exibe apenas o cargo mais compatível com base na Family/Subfamily selecionadas
...
if st.button("🔍 Identificar Cargo"):
    ...
    # Filtra base por family e subfamily
    ...
    with st.container():
        ...
        # Descrições completas — mesmo layout da Job Profile Description
        def render_section(title, content, icon):
            if pd.notna(content) and str(content).strip():
                st.markdown(
                    f"<h5>{icon} {title}</h5><div style='padding:10px 15px; background:#fafaff; border-radius:10px; border:1px solid #eaeaea;'>{content}</div>",
                    unsafe_allow_html=True
                )
